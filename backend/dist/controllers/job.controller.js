...
export const createJob = catchAsync(async (req, res) => {
    const { startDate, frequency, location, duration, title, supportDetails, jobSessions, preference, hourlyRate, jobSessionByClient, } = req.body;
    ...
    if (jobSessionByClient === true &&
        (!jobSessions || jobSessions.length === 0)) {
        throw new AppError("Job Sessions are required when jobSessionByClient is true", 400);
    }
...
export const deleteJob = catchAsync(async (req, res) => {
    ...
    if (hasApplications) {
        throw new AppError("Job cannot be deleted because workers have already applied", 400);
    }
    await job.deleteOne();
    sendResponse(res, {
        success: true,
