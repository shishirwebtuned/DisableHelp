...
export function transformToAPISchema(
  uiData: UIProfileData,
): Partial<WorkerProfileSchema> {
  ...
  if (uiData.personalDetails) {
    ...
    apiData.personalDetails = {
      ...
      additionalTraining: {
        cpr: {
          expiryDate:
            uiData.professionalDetails?.additionalTraining?.cpr?.expiryDate ||
            "",
          file: { url: "", publicId: "" },
          isVerified: false,
        },
        driverLicense: {
          expiryDate:
            uiData.professionalDetails?.additionalTraining?.driverLicense
              ?.expiryDate || "",
          file: { url: "", publicId: "" },
          isVerified: false,
        },
        firstAid: {
          expiryDate:
            uiData.professionalDetails?.additionalTraining?.firstAid
              ?.expiryDate || "",
          file: { url: "", publicId: "" },
          isVerified: false,
        },
    ...
    if (uiData.personalDetails.contactInfo) {
      apiData.locations = [
        {
          id: "loc1",
          name: uiData.personalDetails.contactInfo.name || "",
  ...
  if (uiData.jobDetails) {
    ...
    if (uiData.jobDetails.rates) {
      apiData.rates = [
        ...
        {
          name: "Overnight Rate",
          rate: uiData.jobDetails.rates.overnight || 0,
        },
      ];
    }
  ...
  if (uiData.professionalDetails) {
    ...
    if (uiData.professionalDetails.education) {
      apiData.educationAndTraining = uiData.professionalDetails.education.map(
        (edu) => ({
    ...
    if (uiData.professionalDetails.ndisWorkerScreening) {
      ...
      apiData.ndisWorkerScreening = {
        legal_first_name:
          ndis.legal_first_name ||
          uiData.personalDetails?.personalInfo?.firstName ||
          "",
        legal_middle_name: "",
        legal_last_name:
          ndis.legal_last_name ||
          uiData.personalDetails?.personalInfo?.lastName ||
          "",
  ...
  if (uiData.additionalDetails) {
    ...
    if (add.immunisation) {
      apiData.immunisation = {
        ...
        covidVaccineStatus:
          add.immunisation.covidVaccineStatus || "notVaccinated",
  ...
  if (!apiData.hasOwnProperty("hasActiveAgreement"))
    apiData.hasActiveAgreement = false;
